...
export default function TrollCard({
  ...
  const svgHexagon =
    troll.sign?.fakeColor !== undefined ? (
      ...
      <svg
        ...
        height='61'
        style={{
          filter: `drop-shadow(${hexColor} 0px 0px 4px)`
        }}
        viewBox='0 0 80 70'
        xmlns='http://www.w3.org/2000/svg'
      >
        <path
          stroke={toHex(lightness(hexColor, 90, true))}
          strokeWidth='2'
          fill={styleBlah.color}
  ...
  const svgBox = (
    ...
    >
      <path
        d='M261.5 24.5745L22.5 17.5L17.5 200.426L256.5 207.5L261.5 24.5745Z'
        fill={stroke}
        stroke={stroke}
      />
      <path
        d='M252.5 17.5L13.5 10.5L8.5 191.5L247.5 198.5L252.5 17.5Z'
        fill={styleBlah.borderColor}
        stroke={stroke}
        strokeWidth='3'
      />
      <path
        d='M20.5 2.5L2.5 182.5L240.5 206L258.5 26L20.5 2.5Z'
        stroke={darkstroke}
        fill='transparent'
        strokeWidth='3'
      />
      ...
      >
        <ul>
          {troll.facts?.map((x: string, i: number) => (
            <li key={i}>{x}</li>
          ))}
        </ul>
      </foreignObject>
  ...
  const box = (
    <div
      style={styleBlah}
      className={'box bg-pattern ' + styles.cardBox}
    >
      <img
        src={'/api/cdn/trolls/' + troll.id + '/image.png'}
        alt=''
      />
      {svgBox}
      <div>
        <span className='font-title'>
          {troll.name?.first.toUpperCase()}
          <br />
          {troll.name?.last.toUpperCase()}
        </span>
        {svgHexagon}
      </div>
    </div>
  ...
  if (simple)
    return (
      <Link
        href={'/troll/' + troll.id}
        className={styles.simpleCardLink}
      >
        {box}
      </Link>
    );
