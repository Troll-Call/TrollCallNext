...
export function dialogCompiler(dialog: Pesterlog, jsx: boolean) {
  ...
  let characterColors = dialog.characters.map((char) =>
    toHex(
      Bloods[char.character.sign?.fakeColor ?? char.character.sign?.color ?? 0].colormap.map((x) =>
        Math.round(x * 0xa1)
      )
    )
  );
  ...
  if (dialog.config?.memo) {
    newDialogLog.unshift(
      {
        text: `[C${dialog.config.memoCreator}] ${
          ...
          dialog.characters[dialog.config.memoCreator].character,
          'default'
        )}.`,
        id: '',
        quirk: 'default'
    ...
    if (dialog.characters[0].character.username === dialog.characters[1].character.username)
      newDialogLog.unshift({
        text: '[C0] began trolling ' + dialog.characters[0].character.pronouns[0].split('/')[1] + 'self',
        id: '',
        quirk: 'default'
      });
    else
  ...
  return newDialogLog.map((logitem: Log, index) => {
      ...
      output = reactStringReplace(output, /\[C([0-9][US]?)\]/gm, function (m: string) {
        ...
        return (
          <span style={{ color: characterColors[match] }}>
            {TrollNameRenderer(dialog.characters[match], !m.includes('S'), !m.includes('U'), 1)}
          </span>
        );
      });
    } else {
    ...
    if (logitem.character !== undefined) {
      ...
      output = `${TrollNameRenderer(character, 0, 1, 0)}: ${transformToQuirks(
        output,
        character.character,
        logitem.quirk
      )}`;
