...
export function toInt(color: any) {
  ...
  return (
    (newColor[0] << 16) + (newColor[1] << 8) + newColor[2]
  );
}
...
export function lightness(
  ...
  if (lighter)
    ...
    newColor = [
      clamp(
        Math.round(
          +colorArray[0] - (amt / 100) * +colorArray[0]
        ),
        0,
        255
      ),
      clamp(
        Math.round(
          +colorArray[1] - (amt / 100) * +colorArray[1]
        ),
        0,
        255
      ),
      clamp(
        Math.round(
          +colorArray[2] - (amt / 100) * +colorArray[2]
        ),
        0,
        255
      )
...
export function themeColor(col: any) {
  return Array.apply(null, Array(11)).map((v, i) =>
    lightness(col, Math.abs((i - 5) * 20), i > 5)
  );
}
