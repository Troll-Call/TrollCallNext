...
import { ColorSchema, IDSchema, PolicySchema, PreferenceSchema, Quirk, QuirkSchema } from './generics';
...
export const TrollSchema = yup
  .object({
    ...
    username: yup
      .string()
      .matches(/^(([a-z])[a-z]+)(([A-Z])[a-z]+)$/, 'Username is incorrect')
      .required('Username?'),
    ...
    sign: yup
      .object({
        extended: yup.string().default('Aries').required('Extended Zodiac index (e.g. Virsces, Arrius, Leo)?'),
        color: yup
          .number()
          .min(0, 'Number must not be lower than Aries (0)')
          .max(11, 'Number must not be higher than Pisces (11)')
          .required('No sign?'),
        fakeColor: yup
          .number()
          .min(0, 'Number must not be lower than Aries (0)')
    ...
    species: yup
      .string()
      .matches(/^Troll($|-.+)/, 'Species does not seem to be a subset of trollkind.')
      .default('Troll'),
    pronouns: yup
      .array()
      .of(
        yup
          .string()
          .required('Pronoun?')
          .matches(/\w*\/\w*/, 'Pronoun is incorrectly formatted (correct: [1]/[2], e.x. she/her)')
      )
      .test('unique', 'Duplicate pronouns.', (value) => (value ? value.length === new Set(value)?.size : true))
      .required('Pronouns?'),
    ...
    preferences: yup
      ...
      .max(24, 'Too much preferences (need at or under 24)'),
    facts: yup
      .array()
      .of(yup.string().min(5, 'Fact is too short.').max(140, 'Fact is too long!').required('Fact?'))
      .test('unique', 'Duplicate facts.', (value) => (value ? value.length === new Set(value)?.size : true))
      .required('Facts?')
...
export const ClientTrollSchema = TrollSchema.shape({
  owners: yup
    ...
    .min(1)
    .test('unique', 'Duplicate owners.', (value) => (value ? value.length === new Set(value)?.size : true)),
  // quirks: yup.array().of(
  //   yup.tuple([
