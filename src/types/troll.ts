...
export const TrollSchema = yup.object({
  ...
  pronouns: yup.array().of(yup.string().required("Pronoun?").matches(/\w*\/\w*/, "Pronoun is incorrectly formatted (correct: [1]/[2], e.x. she/her)")).test(
    'unique',
    'Duplicate pronouns.',
  ...
  preferences: yup.array().of(PreferenceSchema).required("Preferences?").min(6, "Too little preferences (need at or above 6)").max(24, "Too much preferences (need at or under 24)"),
  facts: yup.array().of(yup.string().min(5, "Fact is too short.").max(140, "Fact is too long!").required("Fact?")).test(
    'unique',
    'Duplicate facts.',
    (value) => value ? value.length === new Set(value)?.size : true
  ).required("Facts?").min(3, "Too little facts (need at or above 3)").max(5, "Too many facts (need at or under 5)"),
...
export const ClientTrollSchema = TrollSchema.shape({
  owners: yup.array().of(yup.string().min(1).required("Owner?")).min(1).test(
    'unique',
    'Duplicate owners.',
    (value) => value ? value.length === new Set(value)?.size : true
  ),
