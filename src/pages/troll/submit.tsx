...
import { ClientTrollSchema } from "@/types/troll";
            ...
            <Form>
              ...
              <div>
                ...
                <div className="bg-neg-200 border-[1px] border-neg-800">
                  ...
                  {(() => {
                    /* @ts-ignore */
                    var array = allUsers.filter((v) => v.includes(values.owners ? values.owners[selectedElement] : ""));
                    if (array.length <= 0) return (<span className="bg-neg-200 text-neg-600 w-full py-2 px-2">None found.</span>);
                    return array.map((v, i) => {
                      var owners = values.owners ? values.owners[selectedElement] : "";
                      /* @ts-ignore */
                      var stringPlace = v.indexOf(owners);
                      return (
                        <button type="button" className="inline" onClick={() => setFieldValue(`owners[${selectedElement}]`, v)} key={i}>{v.substring(0, stringPlace)}<b>{owners}</b>{v.substring(owners.length + stringPlace, v.length)}</button>
                      )
                    });
                  })()}
                  </div>
              ...
              <div>
                ...
                <span className="input">
                  <Field type="text" name="name.first" placeholder="Johnny" onBlur={(event: FormEvent) => {
                      handleBlur(event);
              ...
              <div>
                ...
                <span className="input">
                  {brandUrl}troll/<Field type="text" name="id" placeholder="jaynne_johnny" onBlur={(event: FormEvent) => {
                      handleBlur(event);
                      setFieldValue('id',event.target.value.replace(/[^a-zA-Z0-9-_]/g, "_").toLowerCase());
                  }} />
                </span>
                <div className="help">User ID will update based on first name, but you can always set one manually. It&apos;s also good practice to put a signature before the User ID, like <b>meowcatheorange_keelez</b>.</div>
              </div>
              <div>
                <div className="label">Username <ErrorMessage name="username">{error}</ErrorMessage></div>
                <span>
                  <Field type="text" name="username" placeholder="genericExamples" />
                </span>
                <div className="help">
                  Your character&apos;s Trollian-style username.
                  {(() => {
                    // @ts-ignore
                    var name = TrollNameRenderer({character:{username:values.username ?? "genericExamples"}}, false, true, false) as string;
                    return nameParallels[name] ? <div className="warning">This username may conflict with the name of {nameParallels[name]}.</div> : <></>
                  })()}
                </div>
              ...
              <div>
                <div className="label">Description ({textLength}/1000) <ErrorMessage name="description">{error}</ErrorMessage></div>
                <span>
              ...
              <div>
                ...
                <span>
                  <Field type="number" name="age" placeholder="6"  onChange={(e) => {handleChange(e);setFieldValue("ageyears", Math.round((e.target.value * 2.16666666667) * 10) / 10)}} />
                </span>
                <div className="help">
                  Your character&apos;s age in Alternian solar sweeps.
                </div>
                <div className="tinyLabel">Age (Years)</div>
                <span>
                  <Field type="number" name="ageyears" placeholder="13" onChange={(e) => {handleChange(e);setFieldValue("age", Math.round((e.target.value / 2.16666666667) * 100) / 100)}} />
                </span>
                <div className="help">
              ...
              <div>
                <div className="label">True Sign <ErrorMessage name="sign.extended">{error}</ErrorMessage></div>
                <span>
                  <select 
                    title="true sign"
                    name="sign.extended"
                    onChange={(e) => {handleChange(e);setFieldValue("sign.color", Math.floor(e.target?.selectedIndex / 24))}}
                    onBlur={handleBlur}
                  >
                    {Object.entries(ExtendedZodiacIndexes).map((theArray, primaryidx) => (
                      <>
                        {theArray[1].map((v, i) => <option key={primaryidx + "." + i} value={v} label={v === theArray[0] ? `-- ${v} --` : v} />)}
                      </>
                    ))}
                  </select>
                </span>
              ...
              <div>
                <div className="label">Fake Sign <ErrorMessage name="sign.fakeColor">{error}</ErrorMessage></div>
                <span>
                  <select 
                  ...
                  >
                    ...
                    {Bloods.map((v, i) => <option key={i} value={i} label={v.sign + " (" + v.name + ")"} />)}
                  </select>
                </span>
                <div className="help">
              ...
              <div>
                <div className="label">Species <ErrorMessage name="species">{error}</ErrorMessage></div>
                <span>
                  <Field type="text" name="species" defaultValue="Troll" placeholder="..." />
                </span>
                <div className="help">Your character&apos;s sub-species of troll. Should either be <b>Troll</b> or <b>Troll-[something]</b>.</div>
              </div>
              <div>
                <div className="label">Pronouns <ErrorMessage name="pronouns">{error}</ErrorMessage></div>
                <FieldArray name="pronouns">
                  {({insert, remove, push}) => (<>
                    {values.pronouns?.map((pronoun, index) => (<>
                      <div className="tinyLabel">Pronoun Set {index} <ErrorMessage name={`pronouns[${index}]`}>{error}</ErrorMessage></div>
                      <span key={index}>
                        <Field onFocus={() => setSelectedPronounElement(index)} type="text" name={`pronouns[${index}]`} placeholder="they/them" />
                        <button type="button" className="ml-2" onClick={() => remove(index)}>Remove Set</button>
                        {index > 0 ? <button type="button" onClick={() => {remove(index);insert(index - 1, pronoun)}}>Up</button> : <></>}
                        {index < values.pronouns.length - 1 ? <button type="button" onClick={() => {remove(index);insert(index + 1, pronoun)}}>Down</button> : <></>}
                      </span>
                    </>))}
                    <span>
                      <button type="button" className="mt-1" onClick={() => push("")}>Add Pronouns</button>
                    </span>
                  </>)}
                </FieldArray>
                <div className="bg-neg-200 border-[1px] border-neg-800">
                  ...
                  {(() => {
                    /* @ts-ignore */
                    var array = PronounsList.filter((v) => v.includes(values.pronouns ? values.pronouns[selectedPronounElement] : ""));
                    if (array.length <= 0) return (<span className="bg-neg-200 text-neg-600 w-full py-2 px-2">None found.</span>);
                    return array.map((v, i) => {
                      ...
                      return (
                        <button type="button" className="inline" onClick={() => setFieldValue(`pronouns[${selectedPronounElement}]`, v)} key={i}>{v.substring(0, stringPlace)}<b>{pronouns}</b>{v.substring(pronouns.length + stringPlace, v.length)}</button>
                      )
                    });
                  })()}
                  </div>
              ...
              <div>
                <div className="label">Gender <ErrorMessage name="gender">{error}</ErrorMessage></div>
                <span>
                  <Field type="text" name="gender" placeholder="Male" />
                </span>
                <div className="help">Your character&apos;s identified gender.</div>
              </div>
              <div>
                <div className="label">Height <ErrorMessage name="height">{error}</ErrorMessage></div>
                <div className="tinyLabel">Height (Inches)</div>
                <span>
                  <Field type="number" name="height" placeholder="63" onChange={(e) => {handleChange(e);setFieldValue("heightmeters", Math.round((e.target.value / 39.37) * 10) / 10)}} />
                </span>
                <div className="help">
                ...
                <span>
                  <Field type="number" name="heightmeters" placeholder="1.6" onChange={(e) => {handleChange(e);setFieldValue("height", Math.round((e.target.value * 39.37) * 10) / 10)}} />
                </span>
                <div className="help">
              ...
              <div>
                <div className="label">Colors <ErrorMessage name="colors">{error}</ErrorMessage></div>
                <FieldArray name="colors">
                  {({insert, remove, push}) => (<>
                    {values.colors?.map((color, index) => (<>
                      <div className="tinyLabel">Color {index} <ErrorMessage name={`colors[${index}]`}>{error}</ErrorMessage></div>
                      <span key={index}>
                        <Field type="color" name={`colors[${index}]`} />
                        <button type="button" className="ml-2" onClick={() => remove(index)}>Remove Color</button>
                        {index > 0 ? <button type="button" onClick={() => {remove(index);insert(index - 1, color)}}>Up</button> : <></>}
                        {index < values.colors.length - 1 ? <button type="button" onClick={() => {remove(index);insert(index + 1, color)}}>Down</button> : <></>}
                      </span>
                    </>))}
                    <span>
                      <button type="button" className="mt-1" onClick={() => push("")}>Add Color</button>
                    </span>
                  </>)}
                </FieldArray>
              ...
              <div>
                ...
                <div className="tinyLabel">Fanart <ErrorMessage name="policies.fanart">{error}</ErrorMessage></div>
                <span>
                  <select 
                    title="fanart"
                ...
                <div className="tinyLabel">Fanart with other characters <ErrorMessage name="policies.fanartOthers">{error}</ErrorMessage></div>
                <span>
                  <select 
                    title="fanartOthers"
                ...
                <div className="help">
                  Do you allow fanart of your character with other characters?
                </div>
                <div className="tinyLabel">Kinning <ErrorMessage name="policies.kinning">{error}</ErrorMessage></div>
                <span>
                  <select 
                    title="kinning"
                ...
                <div className="tinyLabel">Shipping <ErrorMessage name="policies.shipping">{error}</ErrorMessage></div>
                <span>
                  <select 
                    title="shipping"
                ...
                <div className="tinyLabel">Fanfiction <ErrorMessage name="policies.fanfiction">{error}</ErrorMessage></div>
                <span>
                  <select 
                    title="fanfiction"
                ...
                <div className="help">
                  Do you allow fanfiction including your character?
                </div>
              </div>
              <div>
                    ...
                    {values.preferences?.map((preference, index) => (<>
                      ...
                      <span key={index}>
                        ...
                        <select 
                          ...
                          onBlur={handleBlur}
                        >
                          <option value={true} label="♥ (Likes)" />
                          <option value={false} label="♠ (Hates)" />
                        </select>
                      </span>
                      <span>
                        <button type="button" className="ml-2" onClick={() => remove(index)}>Remove Preference</button>
                        {index > 0 ? <button type="button" onClick={() => {remove(index);insert(index - 1, preference)}}>Up</button> : <></>}
                        {index < values.preferences.length - 1 ? <button type="button" onClick={() => {remove(index);insert(index + 1, preference)}}>Down</button> : <></>}
                      </span>
                    </>))}
                    <span>
                      <button type="button" className="mt-1" onClick={() => push({opinion:true,thing:""})}>Add Preference</button>
                    </span>
                  </>)}
                </FieldArray>
                ...
                <FieldArray name="facts">
                  {({insert, form, remove, push}) => (<>
                    ...
                    {values.facts?.map((fact, index) => (<>
                      ...
                      <span>
                        <button type="button" className="ml-2" onClick={() => remove(index)}>Remove Fact</button>
                        {index > 0 ? <button type="button" onClick={() => {remove(index);insert(index - 1, fact)}}>Up</button> : <></>}
                        {index < values.facts.length - 1 ? <button type="button" onClick={() => {remove(index);insert(index + 1, fact)}}>Down</button> : <></>}
                      </span>
                    </>))}
                    <span>
                      <button type="button" className="mt-1" onClick={() => push("")}>Add Fact</button>
                    </span>
                  </>)}
                </FieldArray>
